...
export default function StudyContent({ subjectName }: { subjectName: string }) {
  const [studyData, setStudyData] = useState<StudySubject | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Simulação de API - substitua pela sua API real
  const fetchStudyData = async (): Promise<StudySubject | null> => {
    // Simula delay da API
    await new Promise((resolve) => setTimeout(resolve, 1000));

    // Dados simulados para diferentes matérias
    const mockData: Record<string, StudySubject> = {
      matematica: {
        id: 1,
        name: "Matemática",
        topics: [
      ...
      fisica: {
        ...
        name: "Física",
        topics: [
          { id: 1, title: "Cinemática" },
          { id: 2, title: "Dinâmica" },
          { id: 3, title: "Leis de Newton" },
          { id: 4, title: "Energia e Trabalho" },
        ],
      },
      quimica: {
        id: 3,
        name: "Química",
        topics: [
          { id: 1, title: "Tabela Periódica" },
          { id: 2, title: "Ligações Químicas" },
          { id: 3, title: "Reações Químicas" },
          { id: 4, title: "Estequiometria" },
        ],
      },
    };

    return mockData[subjectName.toLowerCase()] || null;
  };
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setError("Erro ao carregar os dados ou matéria não encontrada.");
    ...
    <div className="container mt-4">
      <div className="card shadow-lg border-0">
        <div className="card-header bg-white border-0 pb-0">
          <h2 className="textocalendario titulo-materia fw-bold mb-0 mt-3 p-2">
            {studyData?.name}
          </h2>
        </div>
